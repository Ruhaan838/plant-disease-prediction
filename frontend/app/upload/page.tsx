  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `Detected: ${disease} with ${confidencePercent}% confidence`,
      })

      // Auto-save to history if enabled
      if (saveToHistory && selectedFile) {
        setIsSaving(true)
        try {
          // Always pass the file to ensure S3 upload
          // Use S3 URL from prediction if available, otherwise pass empty string (file will be uploaded)
          const imageUrl = prediction.s3Url || ""
          await historyApi.saveToHistory(prediction, imageUrl, selectedFile, prediction.s3Key)
          toast({
          ...
          toast({
            title: "Warning",
            description: error instanceof Error ? error.message : "Prediction completed but failed to save to history",
            variant: "destructive",
          })
        } finally {
          setIsSaving(false)
